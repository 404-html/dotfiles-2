...
cat << 'EOF' > $HOME/.zprezto/modules/prompt/functions/prompt_mycustom_setup
#
...
function prompt_steeef_setup {
  ...
  # Define prompts.
  if [ ${UID} -eq 0 ]; then
  PROMPT="$terminfo[bold]%(?:%F{green};)%f:%F{red}:(%?%f) [${_prompt_steeef_colors[4]}%*%f][${_prompt_steeef_colors[4]}%n@%m%f] ${_prompt_steeef_colors[2]}%(4~|%-1~/…/%2~|%3~)%f "'${vcs_info_msg_0_}'"
"'$python_info[virtualenv]'"$ "
  else
  PROMPT="$terminfo[bold]%(?:%F{green};)%f:%F{red}:(%?%f) [${_prompt_steeef_colors[3]}%*%f][${_prompt_steeef_colors[3]}%n@%m%f] ${_prompt_steeef_colors[2]}%(4~|%-1~/…/%2~|%3~)%f "'${vcs_info_msg_0_}'"
"'$python_info[virtualenv]'"$ "
  fi
