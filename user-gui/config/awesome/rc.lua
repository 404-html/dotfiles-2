...
tags = {
names  = { "www", "cli", "pdf", "doc", 5, 6, 7, 8, "mail", "float" },
layout = { layouts[1], layouts[1], layouts[1], layouts[1], layouts[1],
          layouts[1], layouts[1], layouts[1], layouts[1], layouts[12]
}}
...
globalkeys = awful.util.table.join(
    ...
    -- Send focused window do [left|right] tag.
    ...
    awful.key({ "Mod1"            }, "m", function () awful.util.spawn_with_shell("amixer -c 2 sset Mic toggle") end),
    awful.key({ modkey,           }, "End", function () awful.util.spawn_with_shell("amixer set Master toggle") end),
    awful.key({                   }, "XF86Sleep", function () awful.util.spawn("systemctl suspend") end),
    awful.key({                   }, "Print", function () awful.util.spawn("scrot -e 'mkdir -p ~/Pictures/screenshots && mv $f ~/Pictures/screenshots'") end),
    awful.key({ modkey,           }, "Print", function () awful.util.spawn("scrot -u -e 'mkdir -p ~/Pictures/screenshots && mv $f ~/Pictures/screenshots'") end),
    -- Ugly work around.
    -- https://code.google.com/p/xmonad/issues/detail?id=476
    awful.key({ "Shift"           }, "Print", function () awful.util.spawn_with_shell("sleep 0.2;scrot -s -e 'mv $f ~/Pictures/screenshots'") end),
    awful.key({ "Control", "Mod1" }, "Delete", function () awful.util.spawn("killwindow.sh") end),
    awful.key({ "Mod1"            }, "F2", function () awful.util.spawn("krunner") end),
    awful.key({ "Mod1"            }, "F8", function () awful.util.spawn("nvidia-forcepipeline.sh") end),
    awful.key({ modkey,           }, "F9", function () awful.util.spawn("offpad") end),
...
awful.rules.rules = {
    ...
    { rule = { },
      properties = { border_width = 1 , --beautiful.border_width,
                     ...
                     focus = awful.client.focus.filter, -- FUCK FOCUS STEAL
                     raise = true,
...
client.connect_signal("manage", function (c, startup)
    ...
    c:connect_signal("mouse::enter", function(c)
        ...
        --     client.focus = c
        -- end
        if awful.client.focus.filter(c) then client.focus = c end -- Pure mouse focus
    end)
...
r.run(awful.util.getdir("config") .. "/autostart")
